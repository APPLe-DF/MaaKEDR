...
1. 官服 / B 服 / TapTap 三个资源包分别验证启动链路（区服覆盖节点生效）
2. 冷启动（未开进程）与热启动（已开进程）各跑一遍
3. 每日登录奖励等弹窗出现 / 不出现两种路径都验证
4. 启动动画期间截图不早于画面稳定（识别或 `wait_freezes` 生效）
5. 启动失败（如网络错误弹窗）有兜底提示而非卡死
6. 全量回归：启动完成后接领取奖励 / 刷取任务跑一遍无冲突
